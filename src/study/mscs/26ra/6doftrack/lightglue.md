...
- $P_{ij}$ 是正确匹配对的分配概率，范围 $(0,1)$
- $\log P_{ij}$ 是负数，$P_{ij}$ 越接近 $1$，$\log P_{ij}$ 越接近 $0$
- $\sigma_i^A \in (0,1)$ 是点 $i$ 的可匹配性分数

### 训练第二步的置信度分类器是什么

阶段二就是训练一个“够不够自信可以提前收工”的小分类器——每层都问它：现在的匹配结果和最终层一样吗？一样就打 1，不一样就打 0；学好后，只要它说“够了”，网络就提前退出，省时间。